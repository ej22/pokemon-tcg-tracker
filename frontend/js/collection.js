...
function variantLabel(v) {
  if (!v) return '';
  const map = {
    'normal': 'Normal', 'holo': 'Holo', 'reverse_holo': 'Rev. Holo',
    'foil': 'Foil', '1st_edition': '1st Ed.', 'unlimited': 'Unlimited',
  };
  return map[v] || v.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
}

function groupEntriesByCardVariant(entries) {
  const owned   = entries.filter(e => e.quantity > 0);
  const missing = entries.filter(e => e.quantity === 0);

  const groups = new Map();
  for (const e of owned) {
    const key = `${e.card.api_id}::${e.variant || ''}`;
  ...
  const merged = [...groups.values()].map(group => {
    ...
    return { ...group[0], quantity: totalQty, _groupEntries: group };
  });
...
function renderPosterCard(e) {
  ...
  const entryForJson = e._groupEntries ? { ...e, _groupEntries: undefined } : e;
  const entryJson  = JSON.stringify(entryForJson).replace(/"/g, '&quot;');
  const groupIds   = e._groupEntries ? e._groupEntries.map(g => g.id) : null;
  const groupIdsAttr = groupIds ? ` data-group-ids='${JSON.stringify(groupIds)}'` : '';
  ...
  const deleteOnClick = groupIds
    ? `deleteEntries(event, this.closest('[data-entry]').dataset.groupIds)`
    : `deleteEntry(event, ${e.id})`;
  const vLabel = variantLabel(e.variant);

  return `
    <div class="${cardClasses}" role="button" tabindex="0"
         aria-label="${e.card.name}"
         data-entry="${entryJson}"${groupIdsAttr}
      ...
      <div class="poster-overlay">
        ...
        ${(e.card.rarity || vLabel) ? `
        <div class="poster-rarity-row">
          ${e.card.rarity ? `<span class="poster-rarity">${e.card.rarity}</span>` : ''}
          ${vLabel ? `<span class="poster-variant">${vLabel}</span>` : ''}
        </div>` : ''}
...
function renderCollectionFlat(entries) {
  ...
  collectionGrid.innerHTML = sortGroupEntries(groupEntriesByCardVariant(entries)).map(e => {
    if (e.quantity === 0) {
      missingCount++;
    } else {
...
function renderCollectionGrouped(entries) {
  ...
  const htmlParts = sorted.map(([setId, group], idx) => {
    ...
    const cardsHtml = sortGroupEntries(groupEntriesByCardVariant(group.entries)).map(e => {
      if (e.quantity > 0) {
        const pricingOn = entryPricingOn(e);
        const price = pricingOn ? bestPrice(e.prices, e.variant) : null;
        if (price != null) {
...
function removeCardFromDOM(event) {
  ...
  card.remove();
  if (grid) {
    const remaining = grid.querySelectorAll('[data-entry]').length;
    const subtitle = document.getElementById('trade-binder-subtitle');
    if (subtitle) {
      subtitle.textContent = remaining
        ? `${remaining} card${remaining === 1 ? '' : 's'} available for trade`
        : 'Cards available for trade';
    }
    if (!remaining) {
      document.getElementById('trade-binder-empty')?.classList.remove('hidden');
      document.getElementById('trade-binder-summary')?.classList.add('hidden');
    }
  }
}
...
async function toggleForTrade(event, id, currentValue) {
  event.stopPropagation();
  if (currentValue) removeCardFromDOM(event);
  try {
    await requireAuth();
    await apiFetch(`/collection/${id}`, {
      method: 'PUT',
...
async function deleteEntries(event, groupIdsStr) {
  const ids = JSON.parse(groupIdsStr);
  if (!confirm(`Remove all ${ids.length} copies of this card from your collection?`)) return;
  removeCardFromDOM(event);
  try {
    await requireAuth();
    for (const id of ids) {
    ...
    toast(`${ids.length} entries removed`);
    loadCollection();
  } catch (e) {
    if (e.message !== 'Login cancelled') toast(`Error: ${e.message}`, 'error');
  }
