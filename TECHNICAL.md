...
**Collection view** — Netflix-style poster grid. Each card shows artwork, condition chip, quantity badge, price overlay, and a rarity chip badge (`.poster-rarity` — frosted-glass pill, matching the condition chip aesthetic, omitted for cards with no rarity). In full pricing mode, a P&L dot indicates gain/loss vs purchase price. Hover reveals edit/delete buttons. Touch devices use tap-to-reveal. The view can be switched to grouped-by-set mode using the toggle button in the page header. A sort dropdown is always visible in both flat and grouped modes; in flat mode the set-number sort options are disabled (ordering by number has no meaning without set grouping).
